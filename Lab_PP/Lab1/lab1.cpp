...
int main()
{
 ...
 //**********************************************************
	...
	char cByte = 'A'; //Выделили память под cByte и присвоили значение литерала 65 или 0х41
	cByte    = 0x42; // Присвоили значенние литерала 'B', 66
	cByte    = 66;  // Снова Присвоили значенние литерала 'B', 66
	cByte    = -1;  // Присвоили значенние литерала 'я', -1 , 0хff

	unsigned char ucByte = 0x41; //Выделили память под ucByte и присвоили значение литерала 65 или 0х41
	ucByte   = 'B'; // Присвоили значенние литерала 'B', 66, 0х42
	ucByte   =	-1; // Присвоили значенние литерала 'я', 255 , 0хff

	int iInt =  0xffffffff; //Выделили память под iInt и присвоили значение -1

	unsigned int uiInt = 0xffffffff; //Выделили память под uiInt и присвоили значение 4294967295 (максимальное значение для беззнаковых)

	float fFloat = 1.f;  //Выделили память под fFloat и присвоили значение 1 с ПТ на 2Б
	double dDouble = 1.; //Выделили память под dDouble и присвоили значение 1 с ПТ на 4Б
	...
	double d = 0.1234567890123456789123456789; // 		Внесено 0.12345678901234568, число обрезано и округлено до 4Б, точные 15-16 знаков
	float  f = 0.1234567890123456789123456789; // 		Внесено 0.123456791, число обрезано	до 2Б, точные 7-8 знаков


	d	=	1.; // Внесено 		1.0000000000000000, размер 4Б 
	d = 0.999999999999999999999999999999999; // Внесено с округлением до 16 знака		1.0000000000000000, размер 4Б

	stop
	
	// В комментариях напишите результат, возвращаемый оператором sizeof для
	// переменной типа wchar_t (ее размер)
	wchar_t cw = L'Ф'; // 1060, 0х0424 ЮНИКОД
	size_t n = sizeof(cw); // Размер 2Б, результат sizeof=2
	wchar_t cw2 = 'Ф'; //
	stop
...
// **************************************************************
	...
	iInt=1;
	double dDouble1=iInt/3;		// dDouble1 = static_cast<Double> (iInt/3)
	double dDouble2=iInt/3.;	// dDouble1 = (static_cast<Double> (iInt)/3.)
	...
	short sShort;
	dDouble=fFloat=iInt=sShort=cByte=3.3/3;			// dDouble=(fFloat=(iInt=(sShort=(cByte=(3.3/3))))) int 4Б, float 2Б - потеря данных.
	
	cByte=sShort=iInt=fFloat=dDouble=3.3/3;			// cByte=(sShort=(iInt=(fFloat=(dDouble=(3.3/3))))) Пониженеие разрядности хранения числа, может привести к потере точности
	...
	iInt	=	257; // 0x00000101
	cByte	=	iInt; // static_cast<char> (iInt), если число > 127 или < -128, то будет неверное приведение значения

	unsigned char cN1=255, cN2=2, cSum; 
	cSum = cN1 + cN2; // Переполнение при попытке приведения результат сложения из int в unsigned char:  255 это 0хff + 0х02 = 0х00000101
					//	cSum = static_cast<char> (static_cast<int> (cN1) + static_cast<int> (cN2))
	...
	// Напишите явно преобразования, которые неявно выполняет компилятор
	int iSum = cN1+cN2; // Здесь всё ОК cSum = static_cast<int> (cN1) + static_cast<int> (cN2)
	...
	int iSum1= c1 + c2;   // Складываем знаковые числа -1 и 2,  
	int iSum2= uc1 + uc2; // Складываем беззнаковые числа 255 и 2
...
// ***********************************************************
	...
	// Напишите явно преобразования, которые неявно выполняет компилятор
	int nTmp=100, nn=3;
	dDouble=3.3 + nTmp / nn; // dDouble = static_cast<double> (3.3) + static_cast<double> (static_cast<int> (nTmp / nn))
...
// *******************************************************
	...
	TwinSpace::nTwin = 300; // Из пространства имён TwinSpace 300
	...
	int nTwin; // Локальная переменная с мусором
	nTwin=200;	// Локальная переменная 200
	::nTwin++;  // Глобальная 102
	{
		int nTwin; // Локальная переменная подобласти с мусором
		nTwin=-1; // Локальная переменная подобласти -1
		::nTwin++; // Глобальная 103
		TwinSpace::nTwin++; // Из пространства имён TwinSpace 301
	}

	nTwin--; // Локальная переменная 199
...
// *******************************************************
	...
	for (int i=0; i<3; i++)
	{
		static int nStatic1;// инициализируется 0
		nStatic1 = 100; // присваивание отдельно от объявления каждый цикл
		int nLocal1 = 100;
		nStatic1++;
		nLocal1++;
		stop
...
// *******************************************************
...

	{
		...
		// const int nN; // требуется инициализация константы
		// nN = 1; // Константу нельзя модифицировать после инициализации (без использования const_cast)
	}
...
// ********************************************************
	...
	// соответствующей буквы верхнего регистра.
	{
		...
		while (true)
		{
		std::cout << "Введите букву латинского алфавита или ноль для выхода:" << std::endl;
		std::cout << std::endl;
		std::cin >> ch; 
		std::cout << std::endl;
		if (ch == '0') { break; }
		else if ((ch >= 'A') && (ch <= 'Z')) { ch += 0x20; std::cout << "Переводим в верхний регистр: " << ch << std::endl; }
		else if ((ch >= 'a') && (ch <= 'z')) { ch -= 0x20; std::cout << "Переводим в нижний регистр: " << ch << std::endl; }
		else std::cout << "Требуется ввести букву только латинского алфавита!" << std::endl;
		std::cout << std::endl;
		}
	...
	//Если любой другой симол, то вывести сообщение об ошибке
	{
		...
		int	x=0,y=0;
			...
			std::cout << "установите значение x тип char: " << x << std::endl;
			std::cin >> x;
			std::cout << std::endl;
			std::cout << "Введите букву латинского алфавита 'y' или 'n' :" << std::endl;
			std::cout << std::endl;
			...
			{
			case 'y': 
			case 'Y': y = x; std::cout << "y равен: " << y << std::endl; break;
			case 'n':
			case 'N': y = x * 2; std::cout << "y равен: " << y << std::endl; break;
			default: std::cout << "Вы ввели неверные символы!" << std::endl;
				break;
			}
	...
	
	{
		...
		do
		{
			std::cout << "Введите целое значение и покинуть цикл, если значение удовлетворяет условию: \n значение больше или равно 10 и четное.:" << std::endl;
			std::cout << std::endl;
			std::cin >> x;
		} while ((x<10) || (x & 1));  //while ((x<10) || (x % 2 != 0));
	}
	...
	//найти значение x, при котором sum>1.7
	{
		...
		double sum = 0;
			...
			std::cout << "последнее значение х равно: " << x << std::endl;
			std::cin >> x; // Для задержки
	}
